-- ALIVE v3.1 Together security hardening
-- Keep privileged cross-user aggregation outside the exposed public schema.
...
create or replace function private.get_together_summary_internal(p_days integer default 7)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  active_period as (select distinct user_id from activity),
  active_today as (select distinct user_id from activity where occurred_at >= v_today),
  episode_rollup as (
    select count(*)::int as episodes_period,
  ...
  actual_cigarettes as (
    select bu.user_id, bu.baseline_per_day,
      coalesce(sum(te.cigarette_quantity) filter (
        where te.deleted_at is null and te.product_type = 'cigarette' and te.occurred_at >= v_since
  ...
  mechanism_raw as (
    select coalesce(r.mechanism, r.category, 'other') as mechanism,
      count(*)::int as uses,
  ...
  select jsonb_build_object(
    ...
    'baseline', case
      when bs.evaluable >= 3 then jsonb_build_object(
        'evaluable', bs.evaluable, 'below', bs.below_baseline, 'near', bs.near_baseline,
        'above', bs.above_baseline, 'median_delta_pct', round(bs.median_delta_pct, 1), 'suppressed', false
      )
      else jsonb_build_object(
        'evaluable', bs.evaluable, 'below', null, 'near', null, 'above', null,
        'median_delta_pct', null, 'suppressed', true
      )
    end,
    'mechanisms', ms.items,
    'privacy_threshold', 3,
...
revoke all on function private.get_together_summary_internal(integer) from public;
revoke all on function private.get_together_summary_internal(integer) from anon;
grant execute on function private.get_together_summary_internal(integer) to authenticated;
...
set search_path = ''
as $$
  select private.get_together_summary_internal(p_days);
$$;
...
'Public SECURITY INVOKER contract for privacy-safe ALIVE Together aggregates. Privileged aggregation lives in unexposed private schema.';
