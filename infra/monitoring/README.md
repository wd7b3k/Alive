...
|---|---|---|
| `fast` | минута | apex, редирект `www`, отпечаток живой сборки (коммит против `origin/main` и имя чанка против `current`), `/auth/v1/health`, чтение каталога через PostgREST, первый шаг моста Яндекса |
...
Отдельно — отпечаток сборки, без записи в базу и без алертов:

```bash
sudo STATE_DIR=$(mktemp -d) /srv/alive/repo/infra/monitoring/lib/fingerprint.sh
```

Печатает `ok` или `fail` и подробности. Так проверку можно проверить: подменить
`APP_DIR` на каталог с другим бандлом и убедиться, что она падает, а не молчит.
...
**Сначала — работает ли он вообще.** Скопировать юниты и забыть `enable --now` — ошибка
на один пропущенный шаг, и она не проявляется никак: файлы на месте, ручной прогон
проходит, таблица результатов наполняется. Ровно это и случилось между 28 и 30 августа —
таймеры пролежали выключенными два дня.

```bash
systemctl list-timers --all | grep habitoff-check
```

Три строки — расписание есть. Пусто — мониторинга нет, сколько бы скриптов ни лежало в
каталоге. Второй способ увидеть то же самое — раздел «Мониторинг» в админке: проверка,
не отчитавшаяся втрое дольше своего периода, показывается там как «молчит».
