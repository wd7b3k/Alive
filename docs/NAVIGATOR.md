...
- **31.08.2026 · `CURRENT_STATE.md` отрицает наблюдаемость, которая работает.** Раздел
  ...
  является владелец с телефоном». Мониторинг работает с 28.08.2026 (ADR-0013), с 30.08
  виден из админки (ADR-0016). Найдено попутно задачей по бэкапам, чинить отдельно —
  карточка `current-state-otricaet-rabotayuschuyu`.
