...
- **06.09.2026 · пределы одновременной работы объявлены и не соблюдаются.** `doing` = 8
  ...
  пределе 1. Проверки нет — `docs/tasks/T-20260906-02-process-gates.md`.
- **06.09.2026 · три коллизии номеров постановок.** `R-20260905-01`, `T-20260905-11` и
  ...
  заняты номера 004 и 005. Разводится той же задачей `T-20260906-02`.
...
- **05.09.2026 · три ветки несут миграции задним числом.** `task/counter-health`,
  `agent/monitoring-truth` и `agent/source-funnel-rebased` содержат миграции
  `20260831180000` и `20260831190000`, а в базе применены от 05.09.
  `infra/server/apply-migrations.sh` отбирает файлы по отсутствию записи в журнале, а не
  по дате: он их не пропустит и применит сам на ближайшей выкладке — но к более новой
  схеме, против которой их никто не писал. Карточка `migracii-zadnim-chislom-delayut`.
- **05.09.2026 · десять августовских веток каждый раз попадают в опись невыложенного.**
  `origin/v3.0-*`, `agent/v4.0.0-*`, `agent/owner-vision-*`, `agent/r1-*`,
  `agent/v3.1-*`, `agent/product-strategy-*`, `migration/standalone-repo` — история до
  20.08. Они же держат три оставшихся повтора номеров ADR (0004, 0007, 0013) —
  дономерные копии решений, влитых в `main` под 0007, 0008 и 0014. Пометить архивом или
  удалить — решение владельца, карточка `avgustovskiy-arhiv-schitaetsya-nevylozhennoy`.
