# Habitoff — состояние внешней инфраструктуры

Безопасное версионируемое отражение того, что развёрнуто снаружи. Секреты и private
credentials здесь запрещены.

Обновлено 27.08.2026 — после переезда с Cloudflare Pages и Supabase Cloud на
собственный сервер. Причины переезда и отвергнутые альтернативы:
[`decisions/ADR-0007-self-hosted-ru-contour.md`](decisions/ADR-0007-self-hosted-ru-contour.md).

## Имена

Продукт называется **Habitoff**. Репозиторий по историческим причинам остался
`wd7b3k/Alive`, каталоги на сервере — `/srv/alive`, пользователь системы — `alive`.
Переименовывать их не планируется: цена правки путей в скриптах выкладки, бэкапов и
конфигурации выше пользы от единообразия. Если это когда-нибудь изменится, менять надо
все три места сразу.

## GitHub

- Canonical repository: `wd7b3k/Alive`
- Visibility: `private`
- Default branch: `main`
- Прод собирается **только** с `main`.
- Доступ сервера к репозиторию — deploy key с правом записи, заведённый в
  Settings → Deploy keys репозитория. Ключ аккаунта на сервер не копируется: при
  компрометации машины отзывается доступ к одному репозиторию, а не ко всем.

Исторический источник до отделения продукта: `wd7b3k/humanos/projectsv2.0/products/alive/`.
После миграции считается архивным и неканоническим.

Снимок веток здесь больше не ведётся: он устаревал быстрее, чем обновлялся, и создавал
ложное ощущение точности. Актуальное состояние смотреть в самом git.

## Сервер

| | |
|---|---|
| Провайдер | Timeweb Cloud, зона Москва |
| Конфигурация | 4 vCPU / 8 ГБ RAM / 80 ГБ NVMe / 1 Гбит/с |
| Стоимость | 2 760 ₽/мес |
| ОС | Ubuntu 24.04 LTS |
| Пользователь | `alive`, вход только по ключу, `sudo` без пароля |

Ubuntu 24.04, а не 26.04, сознательно: весь стек живёт в контейнерах и от версии хоста не
зависит, поэтому выигрыша от свежей ОС нет, а риск нештатного поведения Docker с новым
ядром есть.

Первичная настройка выполнена cloud-init при создании сервера: пользователь, SSH-ключ,
выключенный парольный вход, `ufw`, `fail2ban`, автообновления безопасности, swap 2 ГБ,
Docker с зеркалом реестра, раскладка каталогов. Настройка через cloud-init, а не руками
после первого входа, закрывает окно в несколько минут, когда root с паролем доступен из
интернета и его сканируют.

### Периметр

Наружу открыты ровно три порта: `22`, `80`, `443`. Проверено сканированием снаружи, а не
чтением `ufw status`.

**Ловушка, о которую ломаются такие установки:** Docker пишет правила в цепочку
`DOCKER-USER` и публикует порты **в обход `ufw`**. Запись `"8000:8000"` в compose откроет
порт всему интернету, а `ufw status` будет показывать, что всё закрыто. Поэтому каждая
публикация порта в этом проекте пишется только в форме `"127.0.0.1:<порт>:<порт>"`.
Ни одного исключения. Проверка — с чужой машины:

```bash
nmap -Pn -p 22,80,443,3000,5432,8000,9999 <IP>
```

### Реестр образов

Docker Hub не отдаёт образы на российские IP, `download.docker.com` тоже. Поэтому:

- Docker поставлен из репозитория Ubuntu (`docker.io`, `docker-compose-v2`);
- в `/etc/docker/daemon.json` прописано зеркало `https://dockerhub.timeweb.cloud`;
- там же ротация логов контейнеров: без неё они съедают диск за месяцы и роняют базу.

## Домен и TLS

- Пользовательский адрес: `https://habitoff.ru`
- Регистратор и DNS: Рег.ру, NS `ns1.reg.ru` / `ns2.reg.ru`
- Записи: `A @` и `A www` на IP сервера
- TTL зоны: `1h` — минимум, который даёт Рег.ру
- `www` отвечает 301 на apex

Cloudflare не участвует ни в одном звене: ни DNS, ни прокси, ни хостинг. Это условие
всего переезда, а не предпочтение.

Сертификаты — Let's Encrypt, выпускает и продлевает Caddy сам. Обращение к
`acme-v02.api.letsencrypt.org` — единственный регулярный внешний вызов инфраструктуры.

## Кто владеет каталогом выкладки

`/srv/alive/repo` принадлежит **`alive:alive`**, и выкладка запускается от `alive`.
Это не деталь вкуса — на ней стоит работоспособность git:

- `git` отказывается читать репозиторий, принадлежащий другому пользователю
  (`fatal: detected dubious ownership in repository at '/srv/alive/repo'`);
- `sudo deploy.sh` сохраняет `SUDO_UID`, и по нему git признаёт каталог своим. Ломается
  не любой root, а запуск из root-сессии, где `SUDO_UID` нет;
- вторая половина той же беды тише: `npm ci` и сборка от root оставляют в репозитории
  тысячи файлов чужого владельца, и следующая выкладка от `alive` падает уже на них.
  30.08.2026 таких файлов осталось 9525, в самом `.git` — ни одного: git отказался
  работать раньше, чем root успел там что-то создать.

Ровно это и произошло 30.08 в 18:02: выкладка из root-сессии получила отказ от каждой
git-команды, не заметила его и опубликовала сборку с пустым отпечатком. Разбор —
`docs/CURRENT_STATE.md` и `infra/deploy/README.md`; скрипт теперь отказывается
запускаться от root.

Исключение, которое остаётся root-овым намеренно: `/srv/alive/backups` — права `600`,
владелец `root`, так их пишет `habitoff-backup`.

## Доступ по ssh

Вход только по ключу: `passwordauthentication no`, `permitrootlogin prohibit-password`,
`AllowUsers alive root`. Файлы — `infra/ssh/`, там же порядок установки и проверки.

Ловушка, на которой здесь уже обожглись: файлы `sshd_config.d/*.conf` читаются в
лексическом порядке, и для большинства ключей **выигрывает первое значение**, а не
последнее. Запрет пароля, положенный 26.08 в `99-alive.conf`, не действовал ни дня —
его перебивал `50-cloud-init.conf` от образа провайдера. К 30.08 в журнале накопилось
3692 неудачных попытки входа и 460 забаненных адресов. Действующее значение показывает
только `sudo sshd -T`, файл на диске не доказывает ничего.

## Раскладка на диске

```
/srv/alive/
  repo/                      git clone, рабочая копия, её открывает VS Code Remote-SSH
  releases/20260827-101717/  собранная статика
  current -> releases/…      симлинк, его отдаёт Caddy
  backups/                   дампы
  deploy.sh                  выкладка
/srv/supabase/
  docker-compose.yml         официальный, не правится
  docker-compose.override.yml наши изменения
  .env                       права 600
  volumes/                   данные Postgres и код функций
```

Разделение существенно: `repo` можно ломать и пересобирать, `current` — то, что видят
люди. Откат — переключение симлинка, полсекунды.

## Веб-сервер

Caddy (репозиторий Cloudsmith, не Ubuntu — в Ubuntu лежит версия 2022 года).
`/etc/caddy/Caddyfile`:

- `/rest/v1/*`, `/auth/v1/*`, `/functions/v1/*` → `127.0.0.1:8000` (шлюз Supabase);
- всё остальное — статика из `/srv/alive/current`, `try_files` на `index.html`;
- `/functions/v1/yandex/*` получает подставленный заголовок `apikey`.

**С 30.08.2026 файл лежит в репозитории:** `infra/caddy/Caddyfile`, и
`/etc/caddy/Caddyfile` является его копией. Порядок установки, проверки и отката —
`infra/caddy/README.md`. Публикуемый ключ Supabase из файла вынесен в
`/etc/habitoff-caddy.env`: он не секрет, но при ротации меняется одно значение на
сервере, а не коммит.

Тогда же исправлены три вещи, которые до этого чинились только руками по ssh:

- **неизвестный путь отдаёт 404**, а не подставленную главную. Список клиентских
  экранов, у которых своего файла нет, сверяется с `src/services/seo.ts` тестом
  `services/caddy-routes.test.ts`: забытый там раздел начал бы отдавать 404 при
  обновлении страницы, и это хуже мягкого 404;
- **`/site.webmanifest` отдаётся как `application/manifest+json`**;
- **`/assets/*` снова кэшируется год.** Причина расхождения оказалась в одной строке:
  `header Cache-Control "no-cache"` стоял без матчера и перезаписывал заголовок всем,
  включая ассеты, — `immutable` выше по файлу не действовал никогда.

### Что сервер отдавал до этой правки, 30.08.2026

Снято снаружи, `curl` к боевому домену. Это не пересказ конфига, а измерение.

| Запрос | Код | Content-Type | Cache-Control |
|---|---|---|---|
| `/` | 200 | `text/html; charset=utf-8` | `no-cache` |
| `/knowledge`, `/links`, `/releases` | 200 | `text/html; charset=utf-8` | `no-cache` |
| `/what-new` (не существует) | 200 | `text/html; charset=utf-8` | `no-cache` |
| `/site.webmanifest` | 200 | **заголовка нет вовсе** | `no-cache` |
| `/sitemap.xml` | 200 | `text/xml; charset=utf-8` | `no-cache` |
| `/version.json` | 200 | `application/json` | `no-cache` |
| `/assets/index-*.js` | 200 | `text/javascript; charset=utf-8` | **`no-cache`** |
| `https://www.habitoff.ru/` | 301 → `https://habitoff.ru/` | — | — |

Таблица оставлена как точка отсчёта: она снята измерением снаружи, а не чтением
конфига, и именно поэтому поймала два расхождения документа с фактом — `no-cache` на
ассетах вместо обещанного `immutable` и полное отсутствие `Content-Type` у манифеста
(`text/plain` в аудите домыслил браузер, сервер не присылал ничего).

Сжатие работает: с `Accept-Encoding: gzip` тот же чанк приезжает сжатым.

Последний пункт неочевиден и важен. Шлюз требует `apikey` на всех путях, а браузер при
переходе по ссылке заголовков не шлёт — и Яндекс, возвращая человека на `callback`, тем
более. Без подстановки вход через Яндекс упирался бы в 401. Ключ публичный по замыслу,
поэтому лежит прямо в конфиге; **при перегенерации ключей его надо обновить и там**.

## Supabase self-hosted

Официальный compose из `supabase/supabase`, каталог `docker/`. Версии образов
зафиксированы тем, что было на момент клонирования, и не выдумываются:

| Сервис | Образ |
|---|---|
| db | `supabase/postgres:17.6.1.136` |
| auth | `supabase/gotrue:v2.189.0` |
| rest | `postgrest/postgrest:v14.12` |
| functions | `supabase/edge-runtime:v1.74.0` |
| meta | `supabase/postgres-meta:v0.96.6` |
| studio | `supabase/studio:2026.08.03-sha-022b374` |
| api-gw | `envoyproxy/envoy:v1.39.0` |

**Оригинальный `docker-compose.yml` не правится.** Все изменения лежат в
`docker-compose.override.yml`: выключение лишних сервисов профилями, привязка портов к
loopback, переменные внешних провайдеров. Вырезанные руками сервисы сделали бы приём
обновлений Supabase ручной работой на полдня.

Чтобы override подхватывался, в `.env` расширен `COMPOSE_FILE`:
`docker-compose.yml:docker-compose.override.yml`. Без этой строки override игнорируется
молча, и все порты уезжают наружу при закрытом на вид `ufw`.

Выключены: `realtime` (подписок на изменения в коде нет), `storage` и `imgproxy` (файлов
пользователей нет), `supavisor` (пул на одно приложение избыточен, PostgREST держит свой).
`analytics` и `vector` в текущей версии compose отсутствуют сами.

Работающий стек занимает около 2 ГБ из 8. Остальное — на сборку и всплески. Постоянно
занятый swap означает, что что-то из выключенного вернулось.

Публикация портов — только на loopback: шлюз `8000`, studio `3000`, Postgres `5432`.
Studio смотреть через туннель со своей машины:

```bash
ssh -L 3000:127.0.0.1:3000 alive@<IP>
```

Отклонение от исходного плана: Postgres всё-таки публикуется на `127.0.0.1:5432`, чтобы
миграции и сверку хэшей можно было гонять с хоста. Наружу он не смотрит. Оговорка: на
хосте `psql` версии 16, сервер 17 — `psql` работает, `pg_dump` надо звать через
`docker exec`.

## Секреты

Все сгенерированы на сервере скриптами `utils/generate-keys.sh` и
`utils/add-new-auth-keys.sh`. Ни одного значения из `.env.example` не осталось.

`.env` с правами `600`, в git — никогда. Ротация `service_role`, висевшая открытым
вопросом на облачном проекте, произошла сама: ключи новые, старые мертвы вместе с
проектом.

В браузер попадает только `SUPABASE_PUBLISHABLE_KEY`. `SERVICE_ROLE_KEY` существует
внутри docker-сети и в `.env`, наружу не выходит ни при каких условиях.

## Вход

Google — переменными `GOTRUE_EXTERNAL_GOOGLE_*` в override. Redirect URI
`https://habitoff.ru/auth/v1/callback` добавлен в Google Cloud Console; старый URI на
`supabase.co` оставлен до окончательного вывода облачного проекта.

Яндекс — собственный мост в edge-функциях. Подробности и причина в
[`AUTH_PROVIDERS.md`](AUTH_PROVIDERS.md).

Почтовый сервис не подключён и **не требуется**: мост выпускает сессию без письма.
Зафиксировано явно, чтобы через месяц никто не начал искать SMTP.

## Бэкапы

Работают с 27.08.2026. Исходники скриптов и юнитов — в `infra/backups/` репозитория,
на сервере они живут в `/usr/local/bin/` и `/etc/systemd/system/`.

| Задание | Когда (UTC / МСК) | Что делает |
|---|---|---|
| `habitoff-backup` | ежедневно 00:30 / 03:30 | дамп, архив конфигов, шифрование, отправка, чистка |
| `habitoff-verify` | воскресенье 01:30 / 04:30 | поднимает базу из копии и сверяет с живой |
| `habitoff-report` | понедельник 06:00 / 09:00 | недельная сводка «всё живо» |

**Что попадает в копию.** `pg_dump -Fc` через `docker exec` (на хосте `psql` 16, сервер 17
— хостовый `pg_dump` откажется по версии) плюс архив с `.env`, обоими compose-файлами и
`Caddyfile`. Конфиги едут не для полноты: без `.env` база не поднимется — там JWT-ключи,
и без них все выданные сессии мертвы.

**Где лежит.** Локально в `/srv/alive/backups`, права 600, семь дней. Наружу — в папку
приложения на Яндекс.Диске, четырнадцать дней. Резервные копии в панели Timeweb выключены
сознательно: копия на том же аккаунте не закрывает сценарий «аккаунт у провайдера
недоступен», а лишний экземпляр личных данных создаёт.

**Локальная копия открытая, уезжающая — зашифрованная** (`gpg --symmetric`, AES256).
На сервере важнее скорость восстановления, а копию у Яндекса надо защитить от самого
Яндекса. Оговорка, которую нельзя опускать: **пароль шифрования лежит на этом же сервере**
— иначе автоматика не смогла бы шифровать. Значит защита работает против чужого доступа
к хранилищу и не работает против взлома сервера.

**Токен Диска — право `cloud_api:disk.app_folder` и только оно.** Токен заперт в одной
папке и не видит остального Диска. Публичная ссылка с доступом на запись для этого не
годится в принципе: сервер действует от имени владельца, а не как посторонний. `rclone`
не используется — его драйвер Яндекса работает с корнем Диска, и ради инструмента
пришлось бы расширять права. Загрузчик свой, `infra/backups/bin/habitoff-yadisk`,
с обновлением протухающего токена.

**Проверка восстановления — не отчёт, а условие.** Раз в неделю сервер поднимает
одноразовый контейнер, создаёт в нём **пустую** базу и разворачивает туда последний дамп,
после чего сверяет с живой: пользователи, таблицы, миграции, каталог фактов. Разворачивать
надо именно в пустую: образ `supabase/postgres` при первом старте сам наполняет базу
`postgres`, и восстановление в неё молча не доносит данные — первая же проверка 27.08.2026
показала ноль пользователей при трёх живых именно поэтому.

**Молчание — сигнал.** Уведомление об ошибке не придёт, если сервер умер целиком, а это
ровно тот случай, ради которого бэкапы существуют. Поэтому раз в неделю приходит
положительная сводка с датой последней копии, возрастом отправки и результатом проверки.
Отсутствие сводки само по себе означает проблему.

Канал уведомлений — свой Telegram-бот, задаётся в `/etc/habitoff/backup.env`. Пока токен
не прописан, сообщения уходят только в системный журнал (`journalctl -t habitoff`), и
тогда конструкция «узнать не заходя на сервер» не работает.

## Наблюдаемость

Дашборда Supabase больше нет. С 28.08.2026 его место занимают собственные проверки:
восемь скриптов в `infra/monitoring/`, три группы по частоте (минута, пять минут, сутки),
запуск systemd-таймерами, история в схеме `ops` той же базы. Решение — ADR-0013, состав
и установка — `infra/monitoring/README.md`.

С 30.08.2026 результат видно из продукта: раздел «Мониторинг» в админке показывает
состояние по частям сервиса и по каждой проверке. Решение — ADR-0016.

**Таймеры включаются отдельным шагом, и этот шаг пропускали.** 30.08.2026 обнаружилось,
что все четыре юнита скопированы в `/etc/systemd/system`, но ни один не был включён:
`enabled=disabled, active=inactive`. Мониторинг не работал по расписанию ни дня, а
таблица результатов при этом выглядела наполненной — в ней лежали ручные прогоны.
Проверять надо не наличие файлов, а расписание:

```bash
systemctl list-timers --all | grep habitoff-check
```

Пустой вывод означает, что мониторинга нет. То же самое теперь видно и на экране:
проверка, не отчитавшаяся втрое дольше своего периода, показывается как «молчит», а
критичная часть в таком состоянии даёт общий вердикт «неизвестно».

Пульс сторожу (`habitoff-heartbeat.timer`) остаётся выключенным, пока в
`/etc/habitoff-monitoring.env` не задан `WATCHDOG_URL`: слать пульс некому, второго
сервера у продукта пока нет.

У всех сервисов в compose стоит `restart: unless-stopped`. Что перезагрузка сервера
поднимает всё сама — **проверяется `reboot`-ом**, а не предполагается.

## Правило расхождения

Если состояние сервера расходится с репозиторием:

1. не считать ручное изменение новой нормой;
2. определить, ошибка это или желаемое изменение;
3. желаемое сначала оформить в репозитории;
4. затем применить на сервере;
5. обновить этот документ.

Прямой `psql` на боевой базе — **только чтение**. Схема и контент меняются исключительно
миграциями. За историю проекта четыре расхождения прода и репозитория, и все родились из
ручной правки; своя база убирает кнопку SQL Editor, но только если её место не займёт
`psql` по SSH.

Проверка встроена в `deploy.sh` и работает при каждой выкладке: число файлов миграций
обязано равняться числу записей в `supabase_migrations.schema_migrations`.
