...
> **26.08.2026 — переезд.** Прод работает на собственном VPS по адресу
> `https://habitoff.ru`. Cloudflare Pages в схеме больше не участвует.
> Всё, что ниже описывает Cloudflare Pages и `alive-aw2.pages.dev`, —
> историческая запись, а не текущее состояние. Не работает вход: порядок
> проверки в `docs/CURRENT_STATE.md`.
