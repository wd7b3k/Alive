...
там, пока человек не поставит ярлык или не откажется. На десктопе вместо кнопки — QR-код
на `habitoff.ru`. Слева показан настоящий файл иконки, тот самый, что появится на
телефоне. Постоянная точка входа остаётся в Профиле. На iPhone честно сказано: первый заход внутри ярлыка попросит войти ещё
