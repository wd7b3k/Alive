...
**Не работает вход.** Известно владельцу. Порядок проверки: Supabase Auth Site URL и
Additional Redirect URLs → Google Cloud OAuth origins и redirect URIs → Яндекс OAuth
redirect URI → `VITE_APP_ORIGIN` в окружении сборки. Диагностика по симптому:
`redirect_uri_mismatch` — провайдер, возврат на старый домен — Supabase или переменная.
Живой вход через Яндекс не проверялся ни разу — проверять отдельно от Google.

**Дизайн-система собрана.** `--r-lime` выведен, роли цвета разделены: teal — действие,
песочный — человек и личное, `--r-ok`/`--r-amber`/`--r-dim` — подтверждённость.
