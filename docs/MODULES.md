...
Владеет длинными разборами на публичных адресах `/knowledge/**`.

- содержание — `content/knowledge/`: реестр кластеров и статьи в Markdown;
- границы владения: **свой текст статьи** — контекст, механизм, разбор, «что это меняет».
  Утверждения о здоровье модулю не принадлежат: они живут в каталоге и цитируются по
  коду, а текст подставляет сборка;
- поверхностей у статьи нет: раскладка карточек по экранам (`flow`, `links`, `today`,
  `public`) остаётся свойством каталога;
- база знаний — **только читатель** каталога. Ни одной миграции, меняющей каталоги, от
  этого модуля не приходит;
