...
Карта — `.claude/agents/README.md`.

**Делегируй, а не делай сам.** Роли — субагенты, и главная сессия обязана их вызывать:

- первым действием любой задачи — `sostoyanie`: снимок состояния командами;
- новая работа, идея, дефект — `postanovka`: файл `R-`/`T-` до того, как кто-то начал делать;
- работа по направлению — роль направления по её описанию (`infra`, `front`, `znaniya`,
  `vidimost`, `redaktor`, `dizayner`, `analitik`, `yurist`, `smm`, `strateg`);
- в конце — `priyomka`: она одна имеет право сказать «сделано» и «на проде».

Если роль не подключилась сама — вызвать явно по имени. Сделать работу роли самому вместо
её вызова — нарушение: ответ без участия ролей считается непроверенным.
