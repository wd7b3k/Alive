...
## Правила работы — в AGENTS.md, роли — в .claude/agents/
...
`docs/INCIDENTS.md` — журнал: что ломалось и какое правило из этого выросло. Перед задачей
читать не нужно; он нужен, чтобы объяснить правило, не удлиняя `AGENTS.md`.

`.claude/agents/` — роли: кто за что отвечает, какой файл кому принадлежит, какая модель.
Карта — `.claude/agents/README.md`. Порядок в сессии: `sostoyanie` первым, `priyomka`
последней, между ними одна роль направления.

## Как устроено состояние работ

Состояние работ живёт на доске, а не в чатах и не в голове.

- `docs/board/cards.json` — карточки: `id`, `title`, `epic`, `priority`, `status`,
  `commits`, `chats`. Поле `chats` хранит чат-сессию, где карточка обсуждалась. Сколько их
  сейчас — считает `node scripts/board.mjs check`, в документ это число не копируется.
