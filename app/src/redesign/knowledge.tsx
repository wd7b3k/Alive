...
import { ARTICLE_CLUSTERS, articleHref, type ArticleEntry } from '../domain/articles';
import { trackAnonEvent, trackEvent } from '../services/analytics';
...
/**
 * Ссылка на разбор. Обычный `<a>`, а не `AppLink`, — и это не оплошность.
 *
 * Страница статьи — отдельный статический документ без React. `AppLink` перехватывает
 * обычный левый клик и отдаёт его роутеру, у которого такого экрана нет: человек остался
 * бы на прежней странице, решив, что ссылка сломана. Переход обязан быть полной загрузкой.
 *
 * Переход отмечается событием: «карточка → разбор» и «раздел → разбор» — это то, ради
 * чего раздел вообще связывают с продуктом, и не знать, ходят ли по этим ссылкам, значит
 * через месяц спорить об этом на ощущениях.
 *
 * Канал записи зависит от того, вошёл ли человек: у участника событие именное и попадает
 * в его воронку, у гостя — анонимное, с идентификатором захода и без имени. Блок один и
 * тот же стоит на публичных и на личных экранах, поэтому выбор делается здесь, а не в
 * шести местах вызова.
 */
function noteOpen(userId: string | null | undefined, event: Parameters<typeof trackAnonEvent>[0]) {
  if (userId) trackEvent(userId, event);
  else trackAnonEvent(event);
}
...
export function ArticleLink({
  ...
  /** Экран, с которого ушли: `knowledge`, `card`, `links`, `today`. */
  from: string;
  cardCode?: string;
  userId?: string | null;
  children?: ReactNode;
    ...
    <a
      ...
      onClick={() =>
        noteOpen(userId, {
          ...
          surface: from,
          metadata: { article: `${article.cluster}/${article.slug}`, card: cardCode ?? null },
        })
      }
    >
...
/**
 * Ссылки на кластеры под карточками раздела.
 *
 * Карточка отвечает одной строкой; человеку с вопросом «сколько это длится» и «что с этим
 * делать» строки мало. До 05.09.2026 вечера этого блока не было, и раздел существовал для
 * поиска, но не для человека.
 */
export function KnowledgeClusters({ userId }: { userId?: string | null } = {}) {
  if (!ARTICLE_CLUSTERS.length) return null;
  return (
    <section className="r-section">
      ...
      <ul className="r-cluster-links">
        {ARTICLE_CLUSTERS.map((cluster) => (
          <li key={cluster.slug}>
            <a
              href={`/knowledge/${cluster.slug}`}
              onClick={() =>
                noteOpen(userId, {
                  event_type: 'article_cluster_open',
                  surface: 'knowledge',
                  metadata: { cluster: cluster.slug },
                })
              }
            >
              {cluster.title}
...
/**
 * Блок «Подробнее» под карточкой и в контексте.
 *
 * Не больше двух-трёх ссылок: список из десяти — это не помощь, а ещё одно место, где
 * надо выбирать. Пусто — блока нет вовсе, а не заголовок над пустотой.
 */
export function ArticleMore({
  ...
  return (
    <div className="r-article-more">
      <p className="r-kicker">{title}</p>
      <ul>
        {articles.map((article) => (
          <li key={`${article.cluster}/${article.slug}`}>
            <ArticleLink article={article} from={from} cardCode={cardCode} userId={userId} />
            <small>{article.question}</small>
          </li>
        ))}
      </ul>
    </div>
