...
// Словарь уровней переехал в domain/evidence-levels.ts: его читает ещё и сборка
// страниц базы знаний, а тянуть туда весь data.ts вместе с клиентом Supabase нельзя.
// Здесь он остаётся доступен по прежнему пути — ни один импорт в приложении не менялся.
export { EVIDENCE_LEVELS };
export type { EvidenceLevel, EvidenceLevelCode };
