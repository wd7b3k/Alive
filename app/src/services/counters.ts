 ...
 * Цель воронки.
 *
 * Одно и то же имя цели уходит в обе системы — иначе сверка невозможна: расхождение
 * между Метрикой и GA нормально в пределах десятка процентов, но только если считается
 * одно и то же событие.
 *
 * Имена цели и этапа собственного слоя при этом НЕ совпадают: `signed_in` против
 * `signed_up`, `landing` против `auth_started`. Это не небрежность — цель называется
 * так, как её заводят в кабинете, а этап так, как он называется у нас. Но пара должна
 * быть записана в одном месте, иначе она разъедется молча: до 05.09.2026 комментарий
 * здесь утверждал, что имена одинаковы, и это было неправдой уже неделю.
 * Единственное место — `STAGE_TO_GOAL` ниже, и его держит тест.
 */
...
/**
 * Этап собственного слоя → цель в кабинетах. Единственное место, где эта пара записана.
 *
 * `Record<FunnelStage, FunnelGoal>` — не украшение типа: если в `FunnelStage` появится
 * этап, а строки здесь не будет, сборка не пройдёт. Обратную сторону — цель, до которой
 * нельзя добраться ни с одного этапа, — компилятор не поймает, её ловит тест.
 */
export const STAGE_TO_GOAL: Record<FunnelStage, FunnelGoal> = {
  landing: 'auth_started',
  signed_in: 'signed_up',
  onboarded: 'onboarded',
  first_episode: 'first_episode',
  episode_with_result: 'episode_with_result',
  repeat_episode: 'repeat_episode',
};

/**
 * С какого дня у счётчиков есть данные.
 *
 * 30.08.2026. Дата взята не из памяти: в `analytics_visitors` первый посетитель с
 * непустым `metrika_client_id` — 30.08 в 12:19 UTC, а получить этот идентификатор можно
 * только у загрузившегося счётчика. В тот же день бандл с непустыми идентификаторами
 * ушёл в прод. Всё, что кабинеты показывают за более ранние даты, — это не «ноль
 * посетителей», а «счётчика не было».
 *
 * Константа существует потому, что на эту ловушку уже наступили: в кабинете Метрики
 * сравнивали период, в котором счётчика ещё не стояло, и делали из пустоты вывод.
 * Дата записана здесь, в `docs/METRICS.md` и подписью на экране аналитики.
 */
export const COUNTERS_LIVE_SINCE = '2026-08-30';
