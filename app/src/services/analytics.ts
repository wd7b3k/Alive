...
/**
 * Веха воронки: запись в свой слой и цель в кабинеты, одним вызовом.
 *
 ...
 *
 * **Событие пишется всегда, отметка спрашивается только перед целью.** Это исправление
 * ошибки, прожившей ровно один день. Первая редакция спрашивала отметку до записи, а
 * отметка хранилась по браузеру, без имени человека, — и второй человек в том же
 * браузере не попадал в воронку вовсе: ни целью, ни собственной строкой. Одна семья,
 * один ноутбук — и половина пилота невидима.
 *
 * Из разделения следует, откуда берётся каждая гарантия. Цель уходит один раз на
 * человека, и это знание браузера — только он видит, что уже отправлял. Свой слой
 * пишет каждый раз, а повторы снимает запрос: `distinct on (человек, этап)`, первая по
 * времени. База знает про человека больше, чем `localStorage`, и дедупликация её работа.
 *
 * Почему отдельное событие, а не пометка существующего. Часть вех уже пишет база
 * триггерами — `onboarding_completed`, `account_created`. Дублировать их отсюда нельзя:
 * воронка посчитает каждый шаг дважды. Но триггер заполняет `funnel_stage` человеческим
 * текстом («первое знакомство завершено»), а не машинным кодом, и сверить его с целью в
 * кабинете невозможно.
 *
...
export function trackStage(
  ...
  if (markFunnelStageOnce(stage, userId)) trackGoal(STAGE_TO_GOAL[stage]);
}
 ...
 * может нажать дважды.
 *
 * Человека здесь ещё нет, поэтому отметка ведётся по посетителю. Войдёт — начнёт
 * считаться по себе, и вехи вошедшего с вехами гостя не смешаются.
 */
export function reachStageGoal(stage: FunnelStage): void {
  if (markFunnelStageOnce(stage)) trackGoal(STAGE_TO_GOAL[stage]);
}
