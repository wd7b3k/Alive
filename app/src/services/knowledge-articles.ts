 ...
 *
 * Главное правило раздела — ADR-0018: **статья не пересказывает утверждение каталога, а
 * цитирует его по коду**. В тексте стоит `{{claim:without_smoking_more_anxious}}`, а на
 * сборке туда подставляется текст, уровень, границы и источник из выгрузки
 * `app/knowledge-catalog.json`. Собственного текста утверждения у статьи нет, поэтому
 * расходиться с каталогом ей нечем.
 *
 * Выгрузка — тот же файл, из которого собираются страницы карточек (ADR-0017). Второго
 * способа читать каталог на сборке не существует: ни `psql`, ни запроса по сети. Из
 * этого следует всё остальное — проверки одинаково работают на сервере выкладки и в CI,
 * а расхождение выгрузки с базой ловится одной командой
 * `node scripts/dump-knowledge-catalog.mjs --check`.
 *
 * Здесь живёт разбор и всё, что проверяется по тексту: структура статьи, редакционный
 * протокол (`docs/EDITORIAL_PROTOCOL_MED.md`), источники, запись об изменениях.
 * Разметка страниц — в `knowledge-article-pages.ts`.
 */
...
export type ClusterRegistry = { clusters: Cluster[] };
...
export const PRIMARY_SOURCE_HOSTS = [
  // --- Научные издания и органы здравоохранения ------------------------------
  'cochrane.org',
  'cochranelibrary.com',
  ...
  // --- Издающий орган и держатель документа ----------------------------------
  //
  // Категория появилась 05.09.2026 вместе с кластером «Что нового». Список выше
  // молчаливо предполагал, что источник — научная публикация; для регуляторного решения
  // и для закона это неверно. У «FDA вернула заявку» и «закон принят 29.04.2026» нет
  // журнальной статьи: первоисточник — сам документ и тот, кто его выпустил.
  //
  // `achievelifesciences.com` — сообщение самого заявителя. Это первоисточник по факту
  // «компания сообщила», и одновременно самое слабое место категории: сторона
  // заинтересована. Статья, которая на него ссылается, обязана сказать об этом прямо —
  // и говорит.
  //
  // `vidal.ru` — справочник лекарственных средств, а не государственный реестр. Он
  // приведён для одного узкого факта: вещество зарегистрировано в России и продаётся.
  // Медицинских выводов из него не делается.
  'gov.uk',
  'federalregister.gov',
  'consultant.ru',
...
export const LATIN_ALLOWED = [
  ...
  'low-tar',
  'Freedom Fund',
  // Кластер «Что нового» разбирает чужие публикации и решения, и названия органов,
  // изданий и компаний в нём — имена собственные. Переводить их значит лишить читателя
  // возможности найти первоисточник, а транслитерировать — придумать написание,
  // которого нет. Список расширяется точечно, а не заменяется разрешением на латиницу.
  'FDA',
...
const FORBIDDEN: { rule: string; pattern: RegExp; why: string }[] = [
  {
    rule: 'дозировка',
    // Доза — это «сколько принять». Содержание вещества на единицу массы — не доза, а
    // свойство изделия: «0,70 мг на грамм табака» — предмет регуляторного стандарта FDA,
    // и статья про этот стандарт обязана назвать число, иначе ей нечего сообщить.
    // Поэтому концентрация из правила исключена, а «мг в сутки» и «мг дважды в день» —
    // нет: это ровно то, что продукт назначать не должен.
    pattern:
      /\d+(?:[.,]\d+)?\s*(?:мг|мкг|миллиграмм[а-яё]*)(?![а-яё])(?!\s*(?:\/\s*(?:г|мл)|(?:[а-яё]+\s+){0,2}на\s+(?:грамм|миллилитр)))|по\s+\d+\s+таблет|\d+\s*раз[а]?\s+в\s+(?:день|сутки)|дозировк|доз[ауы]\s+препарат|принимать\s+по\s/i,
...
export function checkEditorial(article: Article): void {
  ...
  //
  // Адрес убирается в двух видах: внутри ссылки и голым текстом в определении сноски.
  // Второй вид пришёл со статьями кластера «Что нового», где библиография внизу
  // перечислена строками `[^1]: название, издание, дата, адрес`. Адрес не текст: он не
  // читается, а копируется, и требовать от него русского языка бессмысленно.
  const readable = prose
    .replace(/\{\{claim:[a-z0-9_]+\}\}/g, ' ')
    .replace(/\[\^\d+\](:)?/g, ' ')
    .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1')
    .replace(/https?:\/\/\S+/g, ' ')
    // Идентификаторы публикаций — не текст: номер кокрейновского обзора и DOI читатель
    // не читает, а копирует, чтобы найти документ. Требовать от них русского языка так
    // же бессмысленно, как от адреса.
    .replace(/\bCD\d+(?:\.pub\d+)?\b/g, ' ')
    .replace(/\b10\.\d{4,}\/\S+/g, ' ')
...
/**
 * Все коды, которые статья цитирует, обязаны быть в выгрузке каталога.
 *
 * Выгрузка `app/knowledge-catalog.json` содержит только опубликованные карточки, и она
 * закоммичена — значит проверка работает везде одинаково: и на сервере выкладки, и в CI,
 * где базы нет. Второго механизма для этого не заводится (ADR-0018); расхождение
 * выгрузки с базой ловит `node scripts/dump-knowledge-catalog.mjs --check`.
 *
 * Отсутствие кода — отказ, а не пропуск: страница с дырой на месте медицинского
 * утверждения не должна существовать даже минуту.
 */
export function checkClaimsAgainstCatalog(
  article: Article,
    ...
    if (!known(code)) {
      fail(
        ...
        `цитируется утверждение «${code}», которого нет среди опубликованных. ` +
          ...
          'Обновить выгрузку: node scripts/dump-knowledge-catalog.mjs',
      );
    }
  }
 ...
 *
 * `cardCodes` — коды карточек каталога. Они нужны здесь ради единственного, но
 * обязательного гарда: `/knowledge/<код>` и `/knowledge/<кластер>/<слаг>` живут в одном
 * пространстве имён. Вложенный путь `codeFromPath` не считает адресом карточки, поэтому
 * статьи с карточками не сталкиваются никогда; а вот кластер — сегмент первого уровня,
 * и кластер с именем существующего кода перекрыл бы адрес карточки. Адрес карточки
 * менять нельзя (ADR-0017): переехавший адрес теряет всё, что набрал в индексе.
 */
export function readContent(
  ...
  // Служебные сегменты живут на первом уровне, то есть в том же пространстве, что и
  // карточки. Совпадение означало бы, что страница метода перекрыла карточку.
  for (const segment of RESERVED_SEGMENTS) {
    if (cardCodes.has(segment.replace(/-/g, '_'))) {
      fail(
        'app/knowledge-catalog.json',
        `код карточки «${segment}» занят служебным адресом /knowledge/${segment}`,
      );
    }
  }
  ...
  for (const cluster of registry.clusters) {
    ...
    if (cardCodes.has(cluster.slug.replace(/-/g, '_'))) {
      fail(
        'content/knowledge/clusters.json',
        `слаг кластера «${cluster.slug}» совпадает с кодом карточки каталога. ` +
          'Адрес карточки менять нельзя — переименовывать надо кластер',
      );
    }
