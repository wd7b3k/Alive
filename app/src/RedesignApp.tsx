...
function Guided({ session, data, close, saved, initialTrigger }: { session: Session; data: Bootstrap; close: () => void; saved: () => Promise<Bootstrap>; initialTrigger?: string }) {
  ...
  return <Modal wide onClose={() => closeFlow('modal_closed')}>
    ...
    {step === 'complete' && completion && <section className="r-flow"><div className="r-completion-card"><span className="r-status-icon"><Icon name="check" size={24}/></span><h3>{outcome === 'successful_response' ? 'Выбор остался твоим' : 'Опыт сохранён без оценки'}</h3><p>{selectedRanking?.explanation}</p><div className="r-freedom-grid"><div><small>Вернул время</small><strong>{fmt(completion.timeMinutes)} мин</strong></div><div><small>Сохранил деньги</small><strong>{money(completion.moneyRub)}</strong></div><div><small>≈ Сохранил здоровую жизнь</small><strong>{fmt(completion.healthMinutes)} мин</strong></div></div><small>За 7 дней. ≈Здоровая жизнь считается только для подтверждённых невькуренных сигарет по популяционной оценке, а не как личный прогноз.</small></div><div className="r-actions"><ShellButton className="primary" onClick={close}>Вернуться в сегодня</ShellButton></div></section>}
  </Modal>;
}
