...
function Setup({ session, data, done, cancel }: { session: Session; data: Bootstrap | null; done: () => Promise<void>; cancel?: () => void }) {
  ...
  async function save() {
    ...
    if (chosen.cigarette) products.push({ productType: 'cigarette', role: 'target_dependency', baseline: { cigarettes_per_day: Number(cigs || 0), ...(startYear.trim() ? { start_year: year } : {}) }, defaults: { pack_price_rub: Number(pack || 0), pack_size: 20 } });
