...
function Today({ session, data, reload, openFlow, openQuick, openEvening }: { session: Session; data: Bootstrap; reload: () => Promise<void>; openFlow: (trigger?: string) => void; openQuick: () => void; openEvening: () => void }) {
  ...
  const primaryProduct=data.products.find((product)=>product.role==='target_dependency')?.product_type;
  const momentFact=data.release4.awarenessContent.find((item)=>item.content_type==='факт'&&(!primaryProduct||item.product_types.includes(primaryProduct)))??null;
...
function Meanings({ session, data, reload }: { session: Session; data: Bootstrap; reload: () => Promise<void> }) {
  ...
  return <main className="r-page"><section className="r-title meaning"><p className="r-kicker">Зачем</p><h1>Не «почему нельзя», а ради чего становится интереснее жить иначе</h1><p>Зачем — короткая личная опора, которую стоит увидеть в момент, когда мозг предлагает старый автоматический сценарий.</p><ShellButton className="ghost" onClick={()=>setAdding(!adding)}><Icon name="plus" size={18}/> Мой Зачем</ShellButton></section>{adding&&<section className="r-section r-form"><label className="r-field"><span>Заголовок</span><input value={title} onChange={(e)=>setTitle(e.target.value)} placeholder="Я хочу увидеть себя без автоматизма"/></label><label className="r-field"><span>Что это значит для меня</span><textarea value={body} onChange={(e)=>setBody(e.target.value)}/></label><div className="r-actions"><ShellButton className="primary" onClick={add}>Сохранить</ShellButton><ShellButton className="ghost" onClick={()=>setAdding(false)}>Отмена</ShellButton></div></section>}{data.userMeanings.length>0&&<section className="r-section"><div className="r-section-head"><div><p className="r-kicker">Мои</p><h2>Личные опоры</h2></div></div><div className="r-meaning-grid personal">{data.userMeanings.map((m)=><article key={m.id} className={!m.active?'inactive':''}><span className="r-meaning-symbol"><Icon name="meaning" size={25}/></span><h3>{m.title}</h3><p>{m.body}</p><div className="r-meaning-actions"><button onClick={()=>toggle(m)}>{m.active?'Скрыть':'Вернуть'}</button><button onClick={()=>submitMeaning(session,m)}>Предложить в общую базу</button><button onClick={()=>remove(m.id)}>Удалить</button></div></article>)}</div></section>}<section className="r-section"><div className="r-section-head"><div><p className="r-kicker">Библиотека ALIVE</p><h2>Опоры, которые можно примерить на себя</h2></div></div><div className="r-meaning-grid">{data.meanings.map((m)=><article key={m.id}><span className="r-meaning-symbol"><Icon name="meaning" size={25}/></span><h3>{m.title}</h3><p>{m.body}</p></article>)}</div></section><section className="r-section"><div className="r-section-head"><div><p className="r-kicker">Перепись сценария</p><h2>От старого паттерна к новому выбору</h2></div></div><div className="r-scripts">{data.identityScripts.map((s)=><details key={s.code}><summary>{s.title}</summary><div><span><small>Старый сценарий</small>{s.old_pattern}</span><Icon name="arrow"/><span><small>Новый выбор</small>{s.new_choice}</span></div></details>)}</div></section></main>;
}

function FactsPage({ data }: { data: Bootstrap }) {
  const initialTab = new URLSearchParams(window.location.search).get('tab') === 'myths' ? 'миф' : 'факт';
  const [tab, setTab] = useState<'факт' | 'миф'>(initialTab);
  const items = data.release4.awarenessContent.filter((item) => item.content_type === tab);

  function chooseTab(next: 'факт' | 'миф') {
    setTab(next);
    window.history.replaceState({}, '', next === 'миф' ? '/facts?tab=myths' : '/facts');
  }

  return <main className="r-page">
    <section className="r-title">
      <p className="r-kicker">Факты и мифы</p>
      <h1>Знать достаточно, чтобы видеть выбор яснее</h1>
      <p>Факты показывают, что известно о рисках и пользе отказа. Мифы разбирают обещания, которые поддерживают старый ритуал, без страшилок и личных медицинских прогнозов.</p>
      <div className="r-knowledge-tabs">
        <button className={tab === 'факт' ? 'active' : ''} onClick={() => chooseTab('факт')}>Факты</button>
        <button className={tab === 'миф' ? 'active' : ''} onClick={() => chooseTab('миф')}>Мифы</button>
      </div>
    </section>
    {!data.release4.r1Ready ? <section className="r-section"><div className="r-empty compact"><Icon name="shield"/><h3>Материалы временно недоступны</h3><p>ALIVE показывает здесь только опубликованные материалы с проверенным основанием.</p></div></section> : items.length ? <section className={tab === 'факт' ? 'r-fact-grid' : 'r-myth-grid'}>{items.map((item) => <article className={tab === 'факт' ? 'r-fact-card' : 'r-myth-card'} key={item.code}><div className="r-card-meta"><span className={`r-content-kind ${tab === 'миф' ? 'myth' : ''}`}>{tab === 'факт' ? 'Проверенный факт' : 'Разбор мифа'}</span><span>Evidence Registry</span></div><h2>{item.title_ru}</h2><p>{item.hook_ru}</p><details><summary>Подробнее</summary><p>{item.explanation_ru}</p>{item.motivation_ru && <p className="r-content-motivation">{item.motivation_ru}</p>}{item.caveat_ru && <small>{item.caveat_ru}</small>}</details></article>)}</section> : <section className="r-section"><div className="r-empty compact"><Icon name="eye"/><h3>{tab === 'факт' ? 'Факты пока не опубликованы' : 'Разборы мифов пока не опубликованы'}</h3><p>Непроверенный медицинский текст не подставляется вместо отсутствующего материала.</p></div></section>}
  </main>;
}
...
export default function RedesignApp() {
  ...
  else if(path==='/facts')page=<FactsPage data={data}/>;
